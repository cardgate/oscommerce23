...
class cgp_generic {
    ...
    function cgp_generic() {
        ...
        $this->signature = "cardgateplus|cardgateplus|3.09|2.3";
    ...
    function process_button() {
        ...
        foreach ( $order->products as $sKey => $aProduct ) {
            $aProducts [$sKey] ['quantity'] = $aProduct ['qty'];
            $aProducts [$sKey] ['sku'] = $aProduct ['id'];
            $aProducts [$sKey] ['name'] = $aProduct ['name'];
            $aProducts [$sKey] ['price'] = round( $aProduct ['final_price'] * 100, 0 );
            $aProducts [$sKey] ['vat'] = round( $aProduct ['tax'], 0 );
            $aProducts [$sKey] ['vat_inc'] = FALSE;
            $aProducts [$sKey] ['type'] = 1;
        }

        if ( !empty( $order->info ['shipping_cost'] ) && $order->info ['shipping_cost'] > 0 ) {
            if ( MODULE_SHIPPING_FLAT_TAX_CLASS > 0 ) {
                $aShipping_Tax_Rate = tep_get_tax_rate( MODULE_SHIPPING_FLAT_TAX_CLASS, $order->delivery ['country'] ['id'], $order->delivery ['zone_id'] );
                $aProducts [] = array(
                    'quantity' => 1,
                    'sku' => SHIPPING,
                    'name' => MODULE_SHIPPING_FLAT_TEXT_DESCRIPTION,
                    'price' => round( MODULE_SHIPPING_FLAT_COST * 100, 0 ),
        ...
        $process_button_string = tep_draw_hidden_field( 'siteid', $siteID )
                ...
                . $payment_option
                . tep_draw_hidden_field( 'test', $is_test )
                ...
                . tep_draw_hidden_field( 'url_success', HTTP_SERVER . DIR_WS_CATALOG . 'ext/modules/payment/cgp/cgp.php?cgp_notify=true' )
                . tep_draw_hidden_field( 'url_pending', HTTP_SERVER . DIR_WS_CATALOG . 'ext/modules/payment/cgp/cgp.php?cgp_notify=true' )
                . tep_draw_hidden_field( 'url_failure', HTTP_SERVER . DIR_WS_CATALOG . 'ext/modules/payment/cgp/cgp.php?cgp_notify=true' )
                . tep_draw_hidden_field( 'shop_version', PROJECT_VERSION )
                . tep_draw_hidden_field( 'plugin_name', 'OsCommerce CGP' )
                . tep_draw_hidden_field( 'plugin_version', $this->signature )
                . tep_draw_hidden_field( 'hash', $sHashKey )
                . tep_draw_hidden_field( 'extra', $extra )
                . tep_draw_hidden_field( 'cartitems', json_encode( $aProducts, JSON_HEX_APOS | JSON_HEX_QUOT ) );
